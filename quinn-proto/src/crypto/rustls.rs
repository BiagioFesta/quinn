...
impl crypto::HeaderKey for HeaderProtectionKey {
    fn decrypt(&self, pn_offset: usize, packet: &mut [u8]) {
        ...
        let pn_length = PacketNumber::decode_len(first[0]);
        self.xor_in_place(
            &sample[..self.sample_size()],
            &mut first[0],
            &mut rest[pn_offset - 1..pn_offset + pn_length - 1],
        )
        .unwrap();
    }
...
impl crypto::PacketKey for PacketKey {
    fn encrypt(&self, packet: u64, buf: &mut [u8], header_len: usize) {
        ...
        let (payload, tag_buf) = payload_tag.split_at_mut(payload_tag.len() - self.tag_len());
